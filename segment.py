...
def predict(model_to_use_filepath: str,
    ...
    # Load model
    logger.info(f"Load model, with weights {model_to_use_filepath}")
    #    model = kr.models.load_model(model_to_use_filepath)
    model = m.get_unet(input_width=input_image_width, input_height=input_image_height, n_channels=3, 
                       n_classes=1)
    ...
    image_filepaths = glob.glob(f"{input_image_dir}{os.sep}**{os.sep}*{input_ext}", recursive=True)
    ...
    for index, image_filepath in enumerate(sorted(image_filepaths)):
        ...
        image_arr_for_pred = image_arr / 255
        
        # Input of predict must be numpy array with shape: 
        # (nb images, width, height, channels)
        # So add the dimension of the nb images (being 1)
        image_arr_for_pred = np.expand_dims(image_arr_for_pred, axis=0)
    
        # Predict!
        logger.debug("Start prediction")
        image_pred_orig = model.predict(image_arr_for_pred, batch_size=1)
        ...
        # is only one channel
        image_pred = image_pred.reshape((input_image_width, input_image_height))
