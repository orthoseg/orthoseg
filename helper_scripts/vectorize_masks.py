...
def vectorize_masks(
    input_image_dir: Path, output_filepath: Path, projection_if_missing: str
):
    ...
    for input_ext_cur in input_ext:
        image_filepaths.extend(
            glob.glob(f"{input_image_dir}/**/*{input_ext_cur}", recursive=True)
        )
    ...
    for mask_filepath in image_filepaths:
        ...
        with rio.open(mask_filepath) as mask_ds:
            ...
            if mask_transform is None:
                if mask_ds.transform is not None and mask_ds.transform[2] > 0:
                    mask_transform = mask_ds.transform
                else:
                    logger.error(
                        "SKIP FILE: No valid transform info found in filename nor in "
                        f"file metadata ({mask_ds.transform}) for: {mask_filename}"
                    )
                    continue
        ...
        assert isinstance(mask_transform, rio.Affine)
        shapes = rio_features.shapes(mask_arr, mask=mask_arr, transform=mask_transform)
        ...
        if len(geoms) > 0:
            ...
            label_records.append(
                {
                    "geometry": sh_geom.box(xmin, ymin, xmax, ymax),
                    "descr": label_type,
                    "burninmask": 0,
                    "usebounds": 1,
                }
    ...
    labels_gdf = gpd.GeoDataFrame(
        label_records, columns=["geometry", "descr", "burninmask", "usebounds"]
    )
    ...
    # labels_gdf['geometry'] = labels_gdf.geometry.apply(
    #     lambda geom: vh.simplify_visval(geom, 2)
    # )

    # Write result to file
    # logger.debug(
    #     f"Write the {len(geoms_gdf)} geoms, of types "
    #     f"{geoms_gdf.geometry.type.unique()} in geoms_gdf to file"
    # )
