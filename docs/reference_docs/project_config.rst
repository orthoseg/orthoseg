...
.. /home/pierog/github/orthoseg/orthoseg/project_defaults.ini
...
.. confval:: train.dtype_policy
   :type: ``str``
   :default: ``None``

   Keras dtype policy to use during training.
   ...
   If this setting is empty/unset, mixed_float16 is used when a GPU is available,
   otherwise float32 is used.
...
.. confval:: predict.dtype_policy
   :type: ``str``
   :default: ``None``

   Keras dtype policy to use during prediction.
...
.. confval:: postprocess.clip_path
   :type: ``str``
   :default: ``None``

   Clip the result on the layer specified.

   If not specified, no clipping is applied. If a relative path is used it is resolved
   towards the parent dir of the project config file.
...
.. confval:: postprocess.output_style_path
   :type: ``str``
   :default: ``${general:segment_subject}.qml``
