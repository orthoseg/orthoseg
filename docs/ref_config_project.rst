...
.. confval:: train.weights_type
   ...
   - **auto**: use following weights in order, depending on availability: "aerial",
     "imagenet" or None.
   - **aerial**: use weights for the entire model that were trained on aerial images.
     Only available for some architectures. If not available for the architecture used,
     an error is raised.
   - **imagenet**: use imagenet pretrained encoder weights.
   - leave empty: do not use pretrained weights.
   ...
   .. note::
      ...
      You can use the ``segmodels_keras.utils.save_model_weights_notop`` function to save
      the weights of an existing model in the correct format.
